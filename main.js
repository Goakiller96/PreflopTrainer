...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const extraCallRange25 = new Set([
        'K8s', 'K7s', 'K6s', 'K5s',
        'Q9s',
        'J9s',
        '97s',
    ...
    // Диапазон 50/50 - эти руки можно сыграть как колл или рейз
    ...
    function getDefendBBAction(handCode, raiseSize) {
        if (fourbetRange.has(handCode)) return 'raise';
        if (randomRange50.has(handCode)) return 'random'; // Правильный ответ - кнопка 50/50
        if (callRange30.has(handCode)) return 'call';
        if (raiseSize === 2.5 && extraCallRange25.has(handCode)) return 'call';
        return 'fold';
    }
    ...
    function generateDefendBBSituation() {
        ...
        const cards = generateHand();
        const handCode = getHandCode(cards[0], cards[1]);
        const correctAction = getDefendBBAction(handCode, raiseSize);
    ...
    function startNewHand() {
        ...
        console.log(`Режим: ${currentMode}, Рука: ${situation.handCode}, Герой: ${situation.heroPos}, Действие: ${situation.correctAction}`);
    }
    ...
    console.log('✅ Тренажёр готов! Для рук 50/50 доступна отдельная кнопка');
});